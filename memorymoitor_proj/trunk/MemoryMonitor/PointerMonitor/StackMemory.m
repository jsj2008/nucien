...
//void isOK(flex_maybe_object_t* address)
//{
...
//            printf("fucking %d, %lu Class: %s\n", (*address), address ,object_getClassName((__bridge id)address));
...
//    printf("current thread is %u,sp is %llu\n",thread, machineContext.__ss.__sp);
//    printf("current thread is %u,lr is %llu\n",thread, machineContext.__ss.__lr);
//    printf("current thread is %u,fp is %llu\n",thread, machineContext.__ss.__fp);
//    printf("current thread is %u,pc is %llu\n",thread, machineContext.__ss.__pc);
//    printf("current thread is %u,cps is %u\n",thread, machineContext.__ss.__cpsr);
...
//    flex_maybe_object_t *address ;
    ...
    uintptr_t cur_sp = machineContext.__ss.__sp;
    uintptr_t cur_lr = machineContext.__ss.__lr;
    uintptr_t cur_fp = machineContext.__ss.__fp;
    
    //printf("===== %p, %p, %p\n", cur_sp, cur_lr, cur_fp);
    for(; i < 50; i++) {
...
//        internalClassList = objc_copyClassList(&classCount);
        ...
        cur_fp = cur_sp;
        cur_sp = frame.previous;
        
//        printf("current thread is %u,fp is %p\n", thread, cur_fp);
//        printf("current thread is %u,sp is %p\n", thread, cur_sp);
        
        if(cur_sp != 0)
        {
            for(uintptr_t *j = (uintptr_t)cur_fp; j <= (uintptr_t)cur_sp; j = j + 1)
            {
                malloc_printf("StackMemory Pointer, Value: %p, %p\n", j, *j);
                ptrCheck(*j);
            }
        }
    }
