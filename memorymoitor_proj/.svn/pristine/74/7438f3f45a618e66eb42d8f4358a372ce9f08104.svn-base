...
class MallocStackLogEntry
  ...
  def to_s
  	...
  	print ", stackid=" + sprintf("0x%0#{16}X", @stack_identifier)
  	print ", address=" + sprintf("0x%0#{9}X", self.address)
...
def decode_malloc_stack_log_one_recode(fio, no)
	...
	flag = (offset_and_flags_h >> 56)
	stack_id = ((offset_and_flags_h & 0x00FFFFFFFFFFFFFF))
