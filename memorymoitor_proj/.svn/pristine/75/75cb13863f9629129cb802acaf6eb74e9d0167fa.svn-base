...
//            summary.isSomethingEnabled= YES;
    ...
    if (2 == indexPath.section) { // 第2组
        ...
        if (1 == indexPath.row) // 第2组第1行
        {
            ...
            //morefunction = 1;
            int length = hashmap_get_length();
            for(int i = 0; i < length; ++i)
            {
                if((int)hashmap_return_with_index(ptrs_hashmap, i) != -3)
                {
                    base_ptr_t tmp = *(base_ptr_t*)hashmap_return_with_index(ptrs_hashmap, i);
                    ctx = [ctx stringByAppendingFormat:@"stack address %lu with size %d.\n", tmp.address, tmp.size];
                    for(int j = 0; j < tmp.depth; ++j)
                    {
                        ...
                        ctx = [ctx stringByAppendingFormat:@"%d, %s, %lu, %lu\n", j, image.name, image.loadAddr, addr];
