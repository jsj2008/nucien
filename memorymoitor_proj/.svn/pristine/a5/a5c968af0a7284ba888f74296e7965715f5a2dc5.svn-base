...
+ (BOOL)unzipFileAtPath:(NSString *)path
    ...
    zipFile zip = unzOpen_mgnf((const char*)[path UTF8String]);
    ...
    unzGetGlobalInfo_mgnf(zip, &globalInfo);
    
    // Begin Unzip Operations
    
    if (UNZ_OK != unzGoToFirstFile_mgnf(zip)) {
    ...
    do {
        @autoreleasepool {
            if (0 == [password length]) {
                ret = unzOpenCurrentFile_mgnf(zip);
            } else {
                ret = unzOpenCurrentFilePassword_mgnf(zip, [password cStringUsingEncoding:NSASCIIStringEncoding]);
            }
            ...
            ret = unzGetCurrentFileInfo_mgnf(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            
            if (UNZ_OK != ret) {
                success = NO;
                unzCloseCurrentFile_mgnf(zip);
            ...
            unzGetCurrentFileInfo_mgnf(zip, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
            ...
            if (nil != err) {
                DLog(@"ZipArchive Error: %@", err.localizedDescription);
            }
            ...
            if (!fileIsSymbolicLink) {
                ...
                while (fp) {
                    int readBytes = unzReadCurrentFile_mgnf(zip, buffer, 4096);
                ...
                if (fp) {
                    if ([[[fullPath pathExtension] lowercaseString] isEqualToString:@"zip"]) {
                        DLog(@"Unzipping nested .zip file:  %@", [fullPath lastPathComponent]);
                    ...
                    if (0 != fileInfo.dosDate) {
                        ...
                        if (attributes) {
                            if (NO == [fileManager setAttributes:attributes
                                ...
                                DLog(@"[ZipArchive] Failed to set attributes - whilst setting modification date");
                            }
                        }
                    }
                    ...
                    if (0 != permissions) {
                        ...
                        if (NO == [fileManager setAttributes:attributes
                            ...
                            DLog(@"[ZipArchive] Failed to set attributes - whilst setting permissions");
                        }
                    }
                }
            } else {
                ...
                while((bytesRead = unzReadCurrentFile_mgnf(zip, buffer, 4096)) > 0) {
    ...
    for (NSDictionary *dictionary in directoriesModificationDates) {
        if (![[NSFileManager defaultManager] setAttributes:@{NSFileModificationDate: dictionary[@"modDate"]}
                                              ofItemAtPath:dictionary[@"path"]
                                                     error:&err]) {
            DLog(@"[ZipArchive] Set attributes failed for directory: %@.", dictionary[@"path"]);
        }
        if (err) {
            DLog(@"[ZipArchive] Error setting directory file modification date attribute: %@", err.localizedDescription);
        }
    }
...
+ (BOOL)createZipFileAtPath:(NSString *)path
    ...
    BOOL success = NO;
    MgnfZipArchive *zipArchive = [[MgnfZipArchive alloc] initWithPath:path];
...
- (BOOL)open {
    ...
    _zip = zipOpen_mgnf([_path UTF8String], APPEND_STATUS_CREATE);
...
- (BOOL)writeFolderAtPath:(NSString *)path withFolderName:(NSString *)folderName {
    ...
    unsigned int len = 0;
    zipOpenNewFileInZip_mgnf(_zip, [[folderName stringByAppendingString:@"/"] UTF8String], &zipInformation, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_NO_COMPRESSION);
    zipWriteInFileInZip_mgnf(_zip, &len, 0);
    zipCloseFileInZip_mgnf(_zip);
...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(NSString *)fileName {
    ...
    zipOpenNewFileInZip_mgnf(_zip, aFileName, &zipInformation, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION);
    void *buffer = malloc(CHUNK);
    unsigned int len = 0;
    
    while (!feof(input)) {
        len = (unsigned int) fread(buffer, 1, CHUNK, input);
        zipWriteInFileInZip_mgnf(_zip, buffer, len);
    }
...
- (BOOL)writeData:(NSData *)data fileName:(NSString *)fileName {
    ...
    zipOpenNewFileInZip_mgnf(_zip, [fileName UTF8String], &zipInformation, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION);
    zipWriteInFileInZip_mgnf(_zip, data.bytes, (unsigned int)data.length);
    zipCloseFileInZip_mgnf(_zip);
