...
//    printf("current thread is %u, is cps%u\n",thread, machineContext.__ss.__cpsr);
...
//        printf("current thread is %u,sp is %llu\n",thread, machineContext.__ss.__sp);
    ...
    static int fuckingagain = 0;
    for(; i < 128; i++) {
        backtraceBuffer[i] = frame.return_address;
        printf("current thread is %u,sp is %lu\n",thread,frame.curent_sp);
        printf("current thread is %u,fp is %lu\n",thread,frame.previous);
        printf("current thread is %u,lr is %lu\n",thread,frame.return_address);
        internalClassList = objc_copyClassList(&classCount);
//        if(fucking<2)
//        {
//            for(uintptr_t i = frame.curent_sp; i <= frame.previous; i = i + 4)
//            {
//                uintptr_t *curPtr = i;
//                //isOK(curPtr);
//                
